package com.syntax.class30;

import java.util.ArrayList;
import java.util.Iterator;

public class Recap {

	public static void main(String[] args) {
		// collection that can store double obj and can have duplicates and maintain
		// order

		ArrayList<Double> numbers = new ArrayList<>();

		numbers.add(10.99);// boxing
		numbers.add(9.99);
		numbers.add(8.99);
		numbers.add(9.99);
		numbers.add(9.99);
		// numbers.add(7);CE can not store Integer OBj into ArrayList of Double OBj

		Double num = numbers.get(2);
		double num1 = numbers.get(1);// unboxing

		System.out.println("----------Retrieving elements using advance for loop-----------");

		for (double n : numbers) {
			System.out.print(n + " ");
		}

		System.out.println();

		System.out.println(" --------retrieve elements backwards-----");

		for (int i = numbers.size() - 1; i >= 0; i--) {
			double myNum = numbers.get(i);
			System.out.print(myNum);
		}

		System.out.println();
		System.out.println("--------Getting all values using iterator------");

		Iterator<Double> it = numbers.iterator();
		// next();--->gets next Object
		// hasNext(); ---> checks if there is next element
		// remove();--> remove element from the collection

		double b = it.next();
		double b1 = it.next();
		System.out.println(b1);
		System.out.println();
		System.out.print("-------Printing all values using iterator-----------");

		while (it.hasNext()) {
			System.out.println(it.next() + " ");
		}
		System.out.println();
		System.out.println("-------Printing all values using new iterator obj-----------");

		Iterator<Double> doubleIT = numbers.iterator();

		while (doubleIT.hasNext()) {
			double element = doubleIT.next();// unboxing

			if (element < 10) {
				doubleIT.remove();// removing element from collection
			}
		}
		System.out.println(numbers);

	}

}
